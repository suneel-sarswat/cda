...
let rec printmfile oc (m:Extracted.transaction list) k = match m with 
  |[] -> ()
  |[f] -> Printf.fprintf oc "%d,%d,%d\n" f.idb f.ida f.tquantity
  |f::m' -> Printf.fprintf oc "%d,%d,%d;" f.idb f.ida f.tquantity; printmfile oc m' k

let writematching file (m:Extracted.transaction list) k =
...
let rec ploop p i k oc=
  (fun zero succ n -> if n=0 then zero () else succ (n-1))
    ...
    p (Extracted.blist it) (Extracted.alist it) (Extracted.nth k' i Extracted.tau0))  in (printmfile oc (Extracted.mlist output) (k'+1)); output   ) 
    k 
...
let orderbook = str_list_to_instructions (lines_from_files iorderfile)
let m = Extracted.length orderbook;;
Printf.printf "\n%s contains %d instructions.\n" iorderfile m;;
let ofile = "verified_" ^ Sys.argv.(2) ;;
let () = let oc = open_out ofile in
ignore (ploop (Extracted.process_instruction) orderbook m oc) ; close_out oc;;
